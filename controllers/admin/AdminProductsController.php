...
class AdminProductsControllerCore extends AdminController
{
    ...
    public function __construct()
    {
        ...
        $this->available_tabs_lang = array(
            ...
            'DetailImages' => $this->l('DetailImages'),
    ...
    protected function _cleanMetaKeywords($keywords)
    {
        if (!empty($keywords) && $keywords != '') {
            ...
            foreach ($words as $word_item) {
                $word_item = $this->_mergeSpaces($word_item);
                if (!empty($word_item) && $word_item != '') {
                    $out[] = $word_item;
                }
            }
            return trim(((count($out) > 0) ? implode(',', $out) : ''));
        } else {
            return '';
        }
    }
    ...
    */
    protected function _mergeSpaces($string){
    ...
    public function processDetailImage()
    {
        $id_image = (int)Tools::getValue('id_image');
        $image = new Image((int)$id_image);
        if (Validate::isLoadedObject($image)) {
    ...
    {	
		...
		if( isset($_POST['key_tab'])){
			if($_POST['key_tab']=='Features')
			$_POST['submitAddproductAndStay']='';
			
			
			if($_POST['key_tab']=='Combinations')
			$_POST['submitAddproductAndStay']='';
		
			
			if($_POST['key_tab']=='Informations')
			$_POST['submitAddproductAndStay']='';
			
			if($_POST['key_tab']=='Prices')
			$_POST['submitAddproductAndStay']='';
			
			
			if($_POST['key_tab']=='Associations')
			$_POST['submitAddproductAndStay']='';
    ...
    public function ajaxProcessUpdateDetailImagePosition()
    {
        if ($this->tabAccess['edit'] === '0') {
            return die(Tools::jsonEncode(array('error' => $this->l('You do not have the right permission'))));
        }
    ...
    public function ajaxProcessDeleteProductDetailImage()
    {
        $this->display = 'content';
        $res = true;
        /* Delete product image */
        $image = new DetailImage((int)Tools::getValue('id_image'));
        $this->content['id'] = $image->id;
        $res &= $image->delete();
        // if deleted image was the cover, change it to the first one
        /*if (!Image::getCover($image->id_product)) {
            $res &= Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'product_detail_image_shop` image_shop
			SET image_shop.`cover` = 1
			WHERE image_shop.`id_product` = '.(int)$image->id_product.'
			AND id_shop='.(int)$this->context->shop->id.' LIMIT 1');
        }
        ...
        if (!Image::getGlobalCover($image->id_product)) {
            $res &= Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'product_detail_image` i
			SET i.`cover` = 1
			WHERE i.`id_product` = '.(int)$image->id_product.' LIMIT 1');
        }*/
            ...
            if (Validate::isLoadedObject($object)) {
                ...
                if ($object->update()) {
                    ...
                    if (in_array($this->context->shop->getContext(), array(Shop::CONTEXT_SHOP, Shop::CONTEXT_ALL))) {
                        ...
                        if ($this->isTabSubmitted('DetailImages')) {
                            $this->processDetailImageLegends();
                        }
...
/* 		if($_POST){
			file_put_contents('mylog/feature.txt',json_encode($_POST)."\r\n----------------error--------------------------'".date("Y-m-d h:i:s")."---------------------error--------------------------\r\n",FILE_APPEND);	
		} */
    ...
    {	
		...
		$actor = '管理员ID:' .$this->context->employee->id;
		$actor .= '管理员名称:' .$this->context->employee->lastname .'.'.$this->context->employee->firstname ;
		$actor .= '管理员email:' .$this->context->employee->email ;
		$id_product = (int)Tools::getValue('id_product');
		$id_shop=   	Shop::isFeatureActive() && Shop::getContext() == Shop::CONTEXT_SHOP? (int)$this->context->shop->id : 'a.id_shop_default';
		
		if($id_shop == 1 or $id_shop == 2 ){
			$res = Db::getInstance()->executeS('select  a.id_product,a.id_product_attribute,
				GROUP_CONCAT(CONCAT(d.`name`,"--",f.`name`) SEPARATOR "||") as attname,
				 a.available_date ,
				a.default_on from  
				ps_product_attribute_shop  a  
				...
				LEFT JOIN  ps_attribute_lang  f on  b.id_attribute = f.id_attribute 
				LEFT JOIN  ps_attribute_group_lang  d  on c.id_attribute_group  = d.id_attribute_group 
				where   a.id_product = '.$id_product.' and a.default_on = 1 and  a.id_shop = 1
		...
		//产品存在组合 则推送默认组合项到产品详情页面
		if($res){
			
			 $this->context->smarty->assign('default_attribute', $res);
		}else{
			 $this->context->smarty->assign('default_attribute', '');
		}
    ...
    */
    ...
    public function ajaxProcessAddProductDetailImage()
    {
        self::$currentIndex = 'index.php?tab=AdminProducts';
        $product = new Product((int)Tools::getValue('id_product'));
        $legends = Tools::getValue('legend');
        $legends2 = Tools::getValue('legend2');
        ...
        $detail_image_uploader = new HelperImageUploader('detail_image_file');
        $detail_image_uploader->setAcceptTypes(array('jpeg', 'gif', 'png', 'jpg'))->setMaxSize($this->max_image_size);
        $files = $detail_image_uploader->process();

        foreach ($files as &$file) {
            $image = new DetailImage();
            $image->id_product = (int)($product->id);
            $image->position = DetailImage::getHighestPosition($product->id) + 1;
            ...
            foreach ($legends as $key => $legend) {
             ...
             foreach ($legends2 as $key => $legend) {
                if (!empty($legend)) {
                    $image->legend2[(int)$key] = $legend;
                }
            }

            $image->cover = intval(Tools::getValue('detail_type'));
        ...
        die(Tools::jsonEncode(array($detail_image_uploader->getName() => $files)));
    }
     ...
     * @throws SmartyException
     */
    public function initFormDetailImages($obj)
    {
        $data = $this->createTemplate($this->tpl_form);
        ...
        if ((bool)$obj->id) {
            if ($this->product_exists_in_shop) {
                ...
                $count_images = Db::getInstance()->getValue('
					SELECT COUNT(id_product)
					FROM '._DB_PREFIX_.'product_detail_image
					WHERE id_product = '.(int)$obj->id
                );

                $images = DetailImage::getImages($this->context->language->id, $obj->id);

                if(!empty($images)){
                foreach ($images as $k => $image) {
                    $images[$k] = new DetailImage($image['id_image']);
                }}
                ...
                $languages = Language::getLanguages(true);
                $image_uploader = new HelperImageUploader('detail_image_file');
                $image_uploader->setMultiple(!(Tools::getUserBrowser() == 'Apple Safari' && Tools::getUserPlatform() == 'Windows'))
                    ->setTemplate('detail_image_ajax.tpl')
                    ->setUseAjax(true)->setUrl(
                        Context::getContext()->link->getAdminLink('AdminProducts').'&ajax=1&id_product='.(int)$obj->id
                        .'&action=addProductDetailImage');
                $data->assign(array(
                    'countImages' => $count_images,
                    'id_product' => (int)Tools::getValue('id_product'),
                    'id_category_default' => (int)$this->_category->id,
    ...
    public function renderListAttributes($product, $currency)
    {
        ...
        $this->fields_list = array(
          	'id_product_attribute' => array('title' => $this->l('id'), 'align' => 'left'),
            ...
            'available_date' => array('title' => $this->l('有效'), 'align' => 'left')
    ...
    public function processDetailImageLegends()
    {
        if (Tools::getValue('key_tab') == 'DetailImages' && Tools::getValue('submitAddproductAndStay') == 'update_legends' && Validate::isLoadedObject($product = new Product((int)Tools::getValue('id_product')))) {
            $id_image = (int)Tools::getValue('detail_id_caption');
            $language_ids = Language::getIDs(false);
            foreach ($_POST as $key => $val) {
                if (preg_match('/^detailimage_legend_([0-9]+)/i', $key, $match)) {
                    foreach ($language_ids as $id_lang) {
                        if ($val && $id_lang == $match[1]) {
                            Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'product_detail_image_lang SET legend = "'.pSQL($val).'" WHERE '.($id_image ? 'id_image = '.(int)$id_image : 'EXISTS (SELECT 1 FROM '._DB_PREFIX_.'product_detail_image WHERE '._DB_PREFIX_.'product_detail_image.id_image = '._DB_PREFIX_.'product_detail_image_lang.id_image AND id_product = '.(int)$product->id.')').' AND id_lang = '.(int)$id_lang);
                        }
                    }
                }
            }
            ...
            foreach ($_POST as $key => $val) {
                if (preg_match('/^detailimage_legend2_([0-9]+)/i', $key, $match)) {
                    foreach ($language_ids as $id_lang) {
                        if ($val && $id_lang == $match[1]) {
                            Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'product_detail_image_lang SET legend2 = "'.pSQL($val).'" WHERE '.($id_image ? 'id_image = '.(int)$id_image : 'EXISTS (SELECT 1 FROM '._DB_PREFIX_.'product_detail_image WHERE '._DB_PREFIX_.'product_detail_image.id_image = '._DB_PREFIX_.'product_detail_image_lang.id_image AND id_product = '.(int)$product->id.')').' AND id_lang = '.(int)$id_lang);
                        }
                    }
                }
            }
