...
class AdminAttributeGeneratorControllerCore extends AdminController
{
    ...
    {	
        ...
        if (!is_array(Tools::getValue('options'))) {
            ...
            if (count($tab) && Validate::isLoadedObject($this->product)) {
				...
				(SELECT a.id_product,a.id_product_attribute , CONCAT(d.reference,'a',b.id_product_attribute)  as sku   from  ps_product_attribute  a  
	...
	public function comBackup($id_product)
	{	
		$sql =  "select a.*,GROUP_CONCAT(b.id_attribute) as detail from  ps_product_attribute a
		...
		$str = json_encode($res);
		$actor = Context::getContext()->cookie->id_employee .  '|' .  Context::getContext()->cookie->email;
	/* 	echo '<pre>';
		var_dump($str);
		echo '</pre>';
		exit; */
		
		file_put_contents(_PS_ROOT_DIR_ ."/combination_backup/$id_product.txt",$str."\r\n" ."------------$actor----------".date("Y-m-d  H:i:s")."----------------------\r\n",FILE_APPEND);
    }
    ...
    {			
		...
		$is_backup=Db::getInstance()->executeS( "select id_product from  ps_product_attribute_shop 
			where id_product = ".$this->product->id." and  available_date = '1111-11-11' limit 1  " );
			
		$k_backup = Db::getInstance()->executeS( "select id_product from  px_product_attribute
			where id_product = ".$this->product->id." limit 1  " );
