...
class AdminReturnControllerCore extends AdminController
{
    public function __construct()
    {
        ...
        $this->fields_list = array(
			...
			'tracking_num' => array('title' => $this->l('Tracking number'), 'width' => 'auto', 'align' => 'left'),
    ...
    {	
		...
		$rhistory="select a.*,CONCAT(b.lastname,'.',b.firstname) as actor from  px_sql_log  a 
LEFT JOIN  ps_employee  b  on a.id_customer= b.id_employee
where a.content like'%`id_order_return` = $rid%' 
ORDER BY a.date desc  ";
		$rlog = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($rhistory);
		$rreason="select reason from  ps_order_return where id_order_return=   $rid;";
		$rres = Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue($rreason);
		...
		foreach($rlog as $a ){
			...
			if(strpos($a['content'], "`state` = '1'") !== false ){
				
			$alllog.='<span style="color:red">'.$a['actor'].'</span>更新状态为：Waiting for confirmation <span style="color:red">('.$a['date'].')'.'</span><br>'	;
			}
			if(strpos($a['content'], "`state` = '2'") !== false ){
				
			$alllog.='<span style="color:red">'.$a['actor'].'</span>更新状态为：Waiting for package <span style="color:red">('.$a['date'].')'.'</span><br>'	;
			}
			if(strpos($a['content'], "`state` = '3'") !== false ){
			$alllog.='<span style="color:red">'.$a['actor'].'</span>更新状态为：Package received <span style="color:red">('.$a['date'].')'.'</span><br>'	;
				
			}
			if(strpos($a['content'], "`state` = '4'") !== false ){
			$alllog.='<span style="color:red">'.$a['actor'].'</span>更新状态为：Return denied <span style="color:red">('.$a['date'].')'.'</span><br>'	;
				
			}
			if(strpos($a['content'], "`state` = '5'") !== false ){
			$alllog.='<span style="color:red">'.$a['actor'].'</span>更新状态为：Return completed <span style="color:red">('.$a['date'].')'.'</span><br>'	;			
			}
			$pattern = '/reason = \'.*\'/';
			preg_match($pattern,$a['content'],$m);
			if(count($m)>0){
				$mreason = str_replace('reason = \'','',$m[0]);
				$mreason = trim($mreason,"'");
				$alllog .='<span style="color:red">'.$a['actor'].'</span>更新拒绝原因<span style="color:blue">('.$mreason.')</span><span style="color:red">('.$a['date'].')'.'</span><br>';
			}
		...
		//增加包裹确定 时间功能
	/* 	if($this->object->state == 3){
			
			$returntime  = $this->object->date_upd;
		}else{
			
			$returntime  = '未确定';
		}
		$preturntime = '--<span style="font-weight:bold">包裹确定时间：</span><span style="color:red;font-weight:bold">'.$returntime.'</span>'; */
    ...
    public function postProcess()
    {
        ...
        if (Tools::isSubmit('deleteorder_return_detail')) {
            ...
            if ($this->tabAccess['edit'] === '1') {
				//更新退换货原因 
				if(Tools::getValue('reason')!=''){
					
						$reason="UPDATE `ps_order_return` set reason = '".PSql(Tools::getValue('reason'))."' where `id_order_return` = " . (int)(Tools::getValue('id_order_return'));
						Db::getInstance(_PS_USE_SQL_SLAVE_)->execute($reason);				
