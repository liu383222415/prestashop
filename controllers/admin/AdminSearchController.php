    ...
    */
    ...
    protected function initOrderList()
    {
        $this->fields_list['orders'] = array(
            'signmark' => array('title' => $this->l('是否签收'), 'align' => 'center', 'width' => 65),
            'id_order' => array('title' => $this->l('ID'), 'align' => 'center', 'width' => 25),
			'id_shop' => array('title' => $this->l('id_shop'), 'align' => 'center', 'width' => 25),	
    ...
    public function renderView()
    {
        ...
        if (count($this->errors)) {
            ...
            if (isset($this->_list['orders']) && count($this->_list['orders'])) {
				...
				$i=0;
				foreach($this->_list['orders']  as $a ){
					
					$res = Db::getInstance()->getValue("select mark from  px_order_mark where id_cart = " . $a['id_cart']);
					if($res === false ){
						
						//echo  '已签';
						$this->_list['orders'][$i]['signmark'] = '是';
						...
						if($res == '1'){
							...
							//echo  '未签';	
							$this->_list['orders'][$i]['signmark'] = '否';
						}
