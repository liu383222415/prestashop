...
class OrderOpcControllerCore extends ParentOrderController
{
                ...
                if (Tools::isSubmit('method')) {
                    switch (Tools::getValue('method')) {
                        ...
                        case 'makeFreeOrder':
							...
							$checkcart = Db::getInstance()->ExecuteS("select id_order from  ps_orders  where id_cart = ".
    ...
    public function initContent()
    {
        ...
        if($this->context->shop->theme_name=='uniwigs2016-m'){
             $this->removeJS(array(
                ...
                '/themes/uniwigs2016-m/mobile/js/autoload/jquery-1.8.3-min.js'
            ));
             $this->removeCSS(array(
                ...
                '/themes/uniwigs2016-m/css/modules/blocktopmenu/css/superfish-modified.css',
    ...
    {
        if (!$this->isLogged) {
            return '<form id="paypal-express-checkout-form" class="hidden" action="https://www.lavividhair.com/module/paypalusa/expresscheckout?pp_exp_initial=1" method="post" onsubmit="$(\'#paypal_express_checkout_id_product_attribute\').val($(\'#idCombination\').val());$(\'#paypal_express_checkout_quantity\').val($(\'#quantity_wanted\').val());" class="uk-margin-small-bottom">
                <p class="payment_module select_payment"><input type="image" name="image" src="/img/paypal.png" alt="">
                    </p><div class="uk-margin-top">
                    <button id="paypal-express-checkout-btn-product" class="uk-button uk-button-primary" type="submit" name="submit">PLACE YOUR ORDER</button>
                </div></form>';
        }
