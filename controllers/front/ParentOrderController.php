...
class ParentOrderControllerCore extends FrontController
{
    ...
    public function init()
    {
		...
		//存在免运费的折扣号 则推送一个标志到购物车页面  做免运费判断
		if(true){
			$res = Db::getInstance()->getValue('select GROUP_CONCAT(id_carrier)  from  
								ps_carrier where  active = 1 and deleted  =0  and is_code_free = 1 ');

			if( strpos($res  ,$this->context->cart->id_carrier) !== false    ){
				
				
			}else{
				
					foreach($this->context->cart->getCartRules() as $a){
				
						if($this->checkFreeShipping($a['code'])){
...
/* 		echo  'xxxx';
            ...
            if (CartRule::isFeatureActive()) {
                if (Tools::isSubmit('submitAddDiscount')) {
                    if (!($code = trim(Tools::getValue('discount_name')))) {
						...
						$res = Db::getInstance()->ExecuteS("select * from  ps_cart_cart_rule where id_cart =".$this->context->cart->id);
						if($res){

							$this->errors[] = Tools::displayError('Sorry, only one coupon code is allowed for each order.');
    ...
    {	
		...
		//现金券抵消金额
		$cash_amount = $this->getCustomerCash($this->context->cart->id);


        if (  ($this->context->cart->getOrderTotal()-$point_amount) - $cash_amount  <= 0) {
	...
	//获取客户当前现金券金额
	public function  getCustomerCash($id_cart){
		
		$result = Db::getInstance()-> getValue("SELECT amount from  fa_cash_code_cart 
	WHERE	id_cart = $id_cart   ");
	...
	public function  checkFreeShipping($code){
		
		$result = Db::getInstance()-> getValue("select free_shipping from  ps_cart_rule where  code= '$code'");		
		if($result =='1'){
