...
class AuthControllerCore extends FrontController
{
    ...
    {
        parent::initContent();
		//如果是通过折扣券连接注册的  增加 cookie 折扣标记 code
		if(Tools::getValue('code')!=''){
			
			setcookie("code",Tools::getValue('code'));
    ...
    protected function processSubmitLogin()
    {
        ...
        if (empty($email)) {
            ...
            if (isset($authentication->active) && !$authentication->active) {
                ...
                setcookie('uca_cus_id', (int)$customer->id, time() + 86400000, '/', '.uniwigs.com');
                ...
                if (!count($this->errors)) {
                    if ($customer->add()) {
                        ...
                        $this->updateContext($customer);
						 ...
						 //如果是通过折扣券连接注册的 为其创建折扣券标记
						if( isset($_COOKIE["code"])){
							 
							 $cid =(int)$this->context->cookie->id_customer;
							
							Db::getInstance()->execute("INSERT INTO `px_customer_code` ( `id_customer`, `code`, `date_add`) 
							 VALUES ( $cid, '".$_COOKIE["code"]."','".date('Y-m-d H:i:s')."' );");
							 //清除cookie 标记
    ...
    protected function processSubmitCreate()
    {
        if (!Validate::isEmail($email = trim(Tools::getValue('email_create'))) || empty($email)) {
			...
			$this->context->smarty->assign('code',Tools::getValue('code'));
            $_POST['email'] = $email;
			$_POST['code'] =Tools::getValue('code');
    ...
    protected function sendConfirmationMail(Customer $customer)
    {
		...
		if(isset($_COOKIE['code']) and $_COOKIE['code']!=''){
			
			$tpl=  'account_'.$_COOKIE['code'];
		}else{
			$tpl=  'account';
		}
