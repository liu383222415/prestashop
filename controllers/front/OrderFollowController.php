...
class OrderFollowControllerCore extends FrontController
{
    ...
    public function postProcess()
    {
		...
		if (Tools::isSubmit('submitTracking')) {
			...
			$tracking_num = Tools::getValue('returnExpress');
			
			if (empty($tracking_num)) {
                Tools::redirect('index.php?controller=order-follow&errorDetail3');
            }else{
				Db::getInstance()->Execute("update  ps_order_return set tracking_num = '$tracking_num' where id_order_return = $rid"); 
				Tools::redirect('index.php?controller=order-follow');
			}
    ...
    public function initContent()
    {
        ...
        }elseif (Tools::isSubmit('errorDetail3')) {
            $this->context->smarty->assign('errorDetail3', true);
        }
