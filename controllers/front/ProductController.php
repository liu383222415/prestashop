...
class ProductControllerCore extends FrontController
{
    ...
    public function initContent()
    {
        ...
        if (!$this->errors) {
			...
			//如果是积分页面产品 则推送用户积分到页面去
		/* 	if(stristr('aaaaa'.$_SERVER["REQUEST_URI"],'uniwigs-gift-shop-online')){
            ...
            // Assign detail images
            $this->assignProductDetailImages();

            // Assign detail images customer
            $this->assignProductDetailImagesCustomer();

            // Assign detail images colors
            $this->assignProductDetailImagesColors();
			...
			if($id_customer>0 or true){
				$res  =Db::getInstance()->ExecuteS("select  b.id_order_detail,a.id_order ,a.id_customer,b.product_id,b.product_reference,orvm.id_review from  ps_orders a 
													LEFT JOIN  ps_order_detail  b  on a.id_order= b.id_order 
													LEFT JOIN  px_order_item_review orvm on  orvm.id_order_detail= b.id_order_detail 
													where a.id_customer = $id_customer
													and product_id= ".$this->product->id." 
													and a.current_state = 4 
													and  now() < DATE_ADD(a.date_upd,INTERVAL 90 DAY)
													and orvm.id_review  is  null 
													ORDER BY  id_order_detail desc 
													 ");
								
				//limit 1 
				//查询本地是否符合评论条件
				//然后查询远程是否 存在评论 如果存在 则更新本地的评论状态 不存在则继续
				
					 if($res){
						 
						//如果本地 查到可用评论条目 则远程更新一下本地状态 再推送评论条目到前台
						foreach($res as $a){
							//核查远程是否存在此条评论 对应条目 如果存在 则更新本地评论状态。 
							 $ch = curl_init();
							// curl_setopt($ch,CURLOPT_URL,"http://www.rvm.com/checkrvm.php?id_item=".$a['id_order_detail']);
							 curl_setopt($ch,CURLOPT_URL,"http://rvm.uniwigs.com/checkrvm.php?id_item=".$a['id_order_detail']);
							 ...
							 echo "CURL Error:".curl_error($ch);
								...
								if($output=='nothing'){
									...
									//更新本地 
									Db::getInstance(_PS_USE_SQL_SLAVE_)->execute("insert into px_order_item_review(id_order_detail,id_review,review_at,
									date_add)value(". $res->id_order_detail .",". $res->id_review.",". $res->publish_at .",'".date('Y-m-d H:i:s')."')");
						...
						$res  =Db::getInstance()->ExecuteS("select  b.id_order_detail,a.id_order ,a.id_customer,b.product_id,b.product_reference,orvm.id_review from  ps_orders a 
													...
													limit 1 ");
						if($res){
							$this->context->smarty->assign('review', $res);	
							
						}else{
							
							$this->context->smarty->assign('review',false);	
						}
...
$('#you_may_also_like_product').click(function(){ 
    ...
    protected function assignImages()
    {
		...
		$myimg = str_replace('co','pco',_PS_COL_IMG_DIR_);
        $this->context->smarty->assign(array(
     ...
     * Assign detail images
     */
    protected function assignProductDetailImages()
    {
        $images = $this->product->getDetailImages((int)$this->context->cookie->id_lang,(int)$this->context->shop->id,1);
        $product_images = array();
        ...
        /* 		echo $this->product->id;
                exit; */
        if (count($product_images)) {
            $this->context->smarty->assign('product_detail_images', $product_images);
        }
     ...
     * Assign detail images customer
     */
    protected function assignProductDetailImagesCustomer()
    {

        $images = $this->product->getDetailImages((int)$this->context->cookie->id_lang, (int)$this->context->shop->id, 2);
        $product_images_customer = array();

    
        foreach ($images as $k => $image) {
            $product_images_customer[(int)$image['id_image']] = $image;
        }

        //echo $myimg;
        /* 		echo $this->product->id;
                exit; */
        if (count($product_images_customer)) {
            $this->context->smarty->assign('product_detail_images_customer', $product_images_customer);
        }
    }
     ...
     * Assign detail images colors
     */
    protected function assignProductDetailImagesColors()
    {

        $images = $this->product->getDetailImages((int)$this->context->cookie->id_lang, (int)$this->context->shop->id, 3);
        $product_images_color = array();


        foreach ($images as $k => $image) {
            $product_images_color[(int)$image['id_image']] = $image;
        }

        //echo $myimg;
        /* 		echo $this->product->id;
                exit; */
        if (count($product_images_color)) {
            $this->context->smarty->assign('product_detail_images_color', $product_images_color);
        }
    }
...
/* 		echo '<pre>';
        ...
        if (is_array($attributes_groups) && $attributes_groups) {
            ...
            foreach ($attributes_groups as $k => $row) {
                ...
                if(trim(strtolower($groups[$row['id_attribute_group']]['group_name']))=='color' && !isset($groups[$row['id_attribute_group']]['attributes'][$row['id_attribute']])){
                    $colors_num++;
                }
                ...
                foreach ($groups as &$group) {
                    foreach ($group['attributes_quantity'] as $key => &$quantity) {
                        if ($quantity <= 0) {
                            ...
                            if(strtolower($group['group_name'])=='color'){
                                //$colors_num--;
                            }
                        }
                    }
