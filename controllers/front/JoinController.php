...
class JoinControllerCore extends FrontController
{
    public $php_self = 'join';
    public $ssl = true;
    ...
    */
    public function postProcess()
    {
        ...
        if (Tools::isSubmit('submitMessage')) {
            ...
            if (!($from = trim(Tools::getValue('from'))) || !Validate::isEmail($from)) {
            ...
            }elseif (!($customer_name = trim(Tools::getValue('customer_name'))) || !Validate::isName($customer_name)) {
                $this->errors[] = Tools::displayError('The name cannot be blank.');
            }  elseif (!$message) {
                $this->errors[] = Tools::displayError('The message cannot be blank.');
            } elseif (!Validate::isCleanHtml($message)) {
                $this->errors[] = Tools::displayError('Invalid message');
            } elseif (false && (!($id_contact = (int)Tools::getValue('id_contact')) || !(Validate::isLoadedObject($contact = new Contact($id_contact, $this->context->language->id))))) {
                ...
                if ($contact->customer_service) {
                    if ((int)$id_customer_thread) {
                        ...
                        $ct->id_lang = (int)$this->context->language->id;
                        $ct->email = $from;
                        $ct->customer_name = $customer_name;
                        $ct->status = 'open';
                    ...
                    if ($ct->id) {
                        ...
                        if (!$cm->add()) {
                            $this->errors[] = Tools::displayError('An error occurred while sending the message2.');
                        }
                    } else {
    ...
    */
    public function initContent()
    {
        ...
        $this->setTemplate(_PS_THEME_DIR_.'join-us-form.tpl');
    }
