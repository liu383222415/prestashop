...
class SearchControllerCore extends FrontController
{
    ...
    public function initContent()
    {
        ...
        if ($this->instant_search && !is_array($query)) {
			...
			//begin
				// =$this->object2array($search['result']) ;
			
				
				$i=0;
				$myproduct = array();
				//echo count($search['result']);
				foreach ($search['result'] as $a){
					$myproduct[$i]= $a;
					$i++;
				}
				...
				foreach($myproduct as $a){
					...
					$myproduct[$i]['myimg']=$product->getImages(1);
					$i++;
				}
				
			//Tools::debug($myproduct,'bbb');
            $this->context->smarty->assign(array(
                'products' => $myproduct, // DEPRECATED (since to 1.4), not use this: conflict with block_cart module
                'search_products' => $myproduct,
                'nbProducts' => $search['total'],
                'search_query' => $original_query,
                'homeSize' => Image::getSize(ImageType::getFormatedName('home'))));
